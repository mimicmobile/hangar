  ...
  Future refresh() async {
    Utils.getSharedPrefs().then((sp) {
      prefSet.add(PrefSet("Notification", [
        RadioChoicePref<int>(
            "appsPerRow", "Number of apps", "_value_ per row", sp, 7, [6, 7, 8]),
        RadioChoicePref<int>(
            "numRows", "Number of rows", "_value_ rows", sp, 2, [1, 2, 3]),
        RadioChoicePref<int>(
            "numPages", "Number of pages", "_value_", sp, 1, [1, 2, 3]),
        RadioChoicePref<String>(
            "backgroundColor", "Background color", "_value_", sp, "White", ["White", "Material Dark", "Black"])
      ]));
    });
  }
}
